...
static ssize_t goldfish_switch_state_show(struct class_device *dev, char *buf)
{
	struct goldfish_switch  *qs = class_get_devdata(dev);
	return sprintf(buf, "%d\n", qs->state);
}

static ssize_t goldfish_switch_direction_show(struct class_device *dev, char *buf)
{
	struct goldfish_switch  *qs = class_get_devdata(dev);
...
static CLASS_DEVICE_ATTR(state, S_IRUGO | S_IWUSR, goldfish_switch_state_show, goldfish_switch_state_store);
static CLASS_DEVICE_ATTR(direction, S_IRUGO, goldfish_switch_direction_show, NULL);
